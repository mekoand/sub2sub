...
export function summarizeTasks(tasks, { role = 'caller', days = 7, now = Date.now() } = {}) {
  if (!['caller', 'provider'].includes(role) || ![7, 30].includes(days)) throw new Error('Choose caller (Client) or provider (Host), and 7/30 days for statistics.');
  ...
  for (const task of tasks) {
    ...
    for (const round of included) {
      ...
      for (const modelUsage of usage?.models || [usage]) {
        const detail = { taskId: task.taskId, revision: round.revision, startedAt: round.startedAt, endedAt: round.endedAt, status: round.status,
          resource, harness, requestedModel: usage?.requestedModel ?? null, actualModel: modelUsage?.actualModel ?? null,
          ...
          scope: usage?.scope, observedModels: usage?.observedModels, observedAt: usage?.observedAt, tokens: modelUsage?.tokens ?? tokenCounts({}),
  ...
  return {
    ...
    usageNote: 'Native usage from retained task records. Codex covers the main-thread connection; Claude result data covers its query pipeline, including subagents and compaction but not every internal helper. Claude assistant-only data covers partial main-loop input/cache, not output. Codex input includes cache reads; Claude input and cache read/write fields are separate. Reasoning is included in output. Native totals are not sums of overlapping fields. Null means unknown; group tokens sum known fields only and knownFields counts contributing model-rounds. Resource round totals count a multi-model round once. Client/host views represent the same work and must not be added together. Work-copy cleanup preserves usage; explicit record deletion removes history. No account allowance or price is inferred.',
