...
export function modelCapabilities(models, provider = {}, harness = 'codex') {
  ...
  return { protocol: 2, transferProtocol: 3, taskFileUpdates: true, harness, checkedAt: new Date().toISOString(), allowedModels: allowed, limits: transferLimits(provider), models: models.filter(m => allowed === 'all' || allowed.includes(m.model)) };
}

export function requireModel(settings, capabilities) {
  if (capabilities.protocol !== 2 || !Array.isArray(capabilities.models)) throw new Error('Host model capabilities are unconfirmed or incompatible. Upgrade both endpoints before sending task files.');
