...
export function modelCapabilities(models, provider = {}, harness = 'codex') {
  ...
  return { protocol: 2, transferProtocol: 3, harness, checkedAt: new Date().toISOString(), allowedModels: allowed, limits: transferLimits(provider), models: models.filter(m => allowed === 'all' || allowed.includes(m.model)) };
}
