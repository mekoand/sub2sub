...
import { safePath, excluded, materialize, validateFiles, snapshot, copyEntry } from './files.mjs';
...
export async function buildWorkCopy(stateRoot, id, previous, input, changes, expectedPaths) {
  validateFiles(changes.files, { result: true, maxBytes: Infinity, maxFiles: Infinity });
  ...
  try {
    ...
    } else await materialize(work, input, { maxBytes: Infinity, maxFiles: Infinity });
...
export async function restoreCopy(stateRoot, id, saved, paths, limits) {
  ...
  const storage = path.join(saved, 'restoration-files');
  await fs.rm(storage, { recursive: true, force: true });
  const copy = await snapshot(path.join(saved, 'files'), paths, { ...limits, result: true, allowEmpty: true, storage });
