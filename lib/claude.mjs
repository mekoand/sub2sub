...
export async function runClaude({ executable, workspace, temporaryDirectory, threadId, prompt, model, resolvedModel, reasoningEffort, onThread, onProgress, signal }) {
  ...
  const session = openClaude(executable, workspace, {
    ...
    systemPrompt: { type: 'preset', preset: 'claude_code', append: `Execute this delegated sub2sub task only within this work copy. Task network access and host integrations are unavailable. This turn has a 30 minute limit. Save useful stages to disk, leave time for task-appropriate checks, and finish with deliverable paths, checks actually completed and unfinished work with reasons. Node is available at ${JSON.stringify(process.execPath)}. Use ${JSON.stringify(temporaryDirectory || workspace)} for disposable caches; keep deliverables outside the cache directory. The caller checks delivery completeness.` }
  }, input());
  ...
  try {
    ...
    for (;;) {
      ...
      if (message.is_error && !message.user_message_uuid) throw new Error(message.errors?.join('\n') || message.result || 'Claude execution failed before a result was available.');
      if (message.user_message_uuid !== messageId && !message.user_message_uuids?.includes(messageId)) continue;
