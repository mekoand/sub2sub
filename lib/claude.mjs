...
export async function runClaude({ executable, workspace, temporaryDirectory, threadId, prompt, model, resolvedModel, reasoningEffort, onThread, onProgress, onUsage = () => {}, signal }) {
  ...
  try {
    ...
    await Promise.race([
      ...
      new Promise((_, reject) => { startup = setTimeout(() => reject(new Error('Claude execution initialization timed out; no task message was sent. Check the host Claude version and execution settings.')), 15000); })
    ]);
  ...
  if (failure) throw session.diagnostics ? new Error(`${failure.message}\n${session.diagnostics}`, { cause: failure }) : failure;
...
export async function queryClaudeModels(executable, cwd, signal) {
  ...
  if (!version || version[0] < 2 || version[0] === 2 && (version[1] < 1 || version[1] === 1 && version[2] < 263)) throw new Error('Claude Code 2.1.263 or later is required. Update Claude Code before selecting it as the host execution tool.');
  let finish;
  ...
  try {
    ...
    if (account.apiProvider !== 'firstParty' || !account.subscriptionType || account.apiKeySource && account.apiKeySource !== 'none') throw new Error('Sign in to Claude Code with the host Claude subscription before selecting it. API-key execution is not selected automatically.');
...
function openClaude(executable, cwd, options, prompt) {
  ...
  const session = query({ prompt, options: {
    ...
    spawnClaudeCodeProcess: parameters => {
      ...
      // The SDK does not attach its stderr callback to a custom child process.
      // Keep native diagnostics with task errors, never in the shared node log.
      child.stderr.on('data', text => { diagnostics = (diagnostics + text).slice(-4000); });
      exited = new Promise(resolve => child.once('close', resolve));
      return child;
    }
  } });
  return { query: session, get diagnostics() { return diagnostics; }, async close() {
    session.close();
    if (!child) return;
