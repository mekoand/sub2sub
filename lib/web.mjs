...
const actions = new Set(['onboarding', 'caller_settings', 'provider_settings', 'list_models', 'resource_usage', 'start_sharing', 'stop_sharing', 'exit_sharing', 'create_pairing', 'pair_peer', 'authorize_peer', 'list_pairings', 'revoke_pairing', 'cleanup_shared_task', 'cancel_shared_task', 'edit_peer', 'delete_peer', 'check_peer', 'task_status', 'cancel_task', 'collect_result', 'finish_task', 'web_management']);
const security = {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/task') {
        ...
        json({ taskId: id, responseFile: saved.resultDirectory ? path.join(saved.resultDirectory, 'response.md') : undefined,
          ...
          savedAt: saved.savedAt, deliveryPending: saved.deliveryPending, delivery }); return;
      }
