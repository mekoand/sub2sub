...
export async function manageTaskHistory({ executable, root, taskId, threadId, retain = false, archive, signal }) {
  ...
  const request = (method, params) => {
    ...
    return new Promise((resolve, reject) => {
      const id = ++sequence;
      const timer = setTimeout(() => { pending.delete(id); reject(new Error(`Codex ${method} timed out. History operation may be partial; inspect the same task before retrying.`)); }, 30000);
  ...
  lines.on('line', line => {
    try {
      ...
      if (message.error) item.reject(new Error(`Codex ${item.method}: ${JSON.stringify(message.error)}. Native history operation is incomplete; retry after resolving this error.`));
  ...
  try {
    ...
    await request('initialize', { clientInfo: { name: 'sub2sub-cleanup', version: '0.5.3' }, capabilities: { experimentalApi: true } });
    ...
    for (const archived of [false, true]) {
      ...
      do {
        ...
        for (const summary of page.data) {
          if (archive !== undefined && summary.id !== threadId) continue;
          ...
          if (thread.id === threadId) previouslyArchived = archived;
        }
        cursor = page.nextCursor;
      } while (cursor);
    }
    if (archive !== undefined) {
      if (!targets.has(threadId)) throw new Error('The original native task session was not found. No replacement session was created.');
      if (previouslyArchived !== archive) await request(archive ? 'thread/archive' : 'thread/unarchive', { threadId });
      return { status: archive ? 'archived' : 'visible', checkedAt: new Date().toISOString() };
    }
