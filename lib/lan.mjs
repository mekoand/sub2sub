...
export function lanRequest(peer, route, input, onProgress = () => {}, signal, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const gzip = streaming && route === '/rpc' && peer.transferCompression === 'gzip' && input.snapshot;
    ...
    try {
      ...
      if (req.url === '/local') {
        ...
        if (input.action === 'network') {
          ...
          if (Object.keys(input).some(key => !['action', 'protocol', 'harness', 'taskId', 'snapshot', 'prompt', 'revision', 'discardPaths', 'model', 'reasoningEffort', 'cleanup', 'limits', 'syncId', 'details', 'retentionPolicy'].includes(key))) throw new Error('Unknown host request field.');
