...
function pairingRules(peer) {
  ...
  return { maxConcurrent: limit, expiresAt, authorizationStatus: expiresAt !== null && Date.now() >= Date.parse(expiresAt) ? 'expired' : 'active' };
}
...
function sharingAddress(input, config) {
  ...
  if (!input.address && !config.provider?.address && choices.length !== 1) throw new Error(`Select a private-network address with start_sharing.address. Available: ${choices.join(', ') || 'none; connect to your private network first'}.`);
  return address(input.address || config.provider?.address || choices[0]);
}
...
export function lanRequest(peer, route, input, onProgress = () => {}, signal, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (Buffer.byteLength(body) > WIRE_BYTES + (route === '/local' ? 8192 : 0)) { done(new Error('LAN request exceeds supported transport size.')); return; }
    request = https.request({
      hostname: peer.host, port: peer.port, path: route, method: 'POST', agent: false,
      rejectUnauthorized: false, minVersion: 'TLSv1.2',
      headers: { 'content-type': 'application/json', 'content-length': Buffer.byteLength(body), ...(peer.localDeviceName ? { 'x-sub2sub-device-name': encodeURIComponent(peer.localDeviceName) } : {}), ...(peer.token ? { authorization: `Bearer ${peer.token}` } : {}) }
    }, response => {
    ...
    request.on('error', error => done(Object.assign(new Error(`LAN connection failed: ${error.message}. Check that the provider has sharing enabled.`, { cause: error }), { beforeSend: !sent })));
    ...
    connectTimer = setTimeout(() => done(Object.assign(new Error('LAN connection timed out. Check the provider address and sharing status.'), { beforeSend: !sent })), options.connectTimeout ?? 15000);
    timer = setTimeout(() => done(new Error('Connection request timed out. Query the task status before resuming.')), options.requestTimeout ?? 31 * 60 * 1000);
  });
}
...
export class Sharing {
  ...
  async requestNetwork({ peer, route, request }, onProgress) {
    ...
    try {
      ...
      return { ...result, connectionRoute: 'tailcat' };
    } catch (error) {
  ...
  async configurePairing(input) {
    taskId(input.pairId);
    if (Object.keys(input).some(key => !['pairId', 'maxConcurrent', 'expiresAt'].includes(key))) throw new Error('Unknown connection setting.');
    let result;
    const read = config => {
    ...
    if (Object.keys(input).some(key => key !== 'pairId')) {
      await this.store.update(config => {
        ...
        result = { pairId: input.pairId, name: peer.name, ...rules };
      });
    } else {
  ...
  requirePairingCapacity(config, pairId) {
    ...
    if (rules.authorizationStatus === 'expired') throw new Error('Connection authorization has expired. Ask the provider to extend this connection; existing tasks can still be queried, cancelled and collected.');
  ...
  async open(input) {
    try {
      ...
      this.tlsOptions = { ...identity, minVersion: 'TLSv1.2', requestTimeout: 60000, headersTimeout: 15000, maxHeaderSize: 8192 };
      const server = https.createServer(this.tlsOptions, (req, res) => { void this.handle(req, res); });
      await new Promise((resolve, reject) => { server.once('error', reject); server.listen(port, host, resolve); });
      this.server = server;
  ...
  async handle(req, res, viaTailcat = false) {
    ...
    try {
      ...
      if (req.url === '/rpc') {
        ...
        if (!pairId) throw Object.assign(new Error('Caller is not paired or its authorization was revoked.'), { code: 'PAIRING_REVOKED' });
      }
      ...
      for await (const chunk of req) {
        bytes += Buffer.byteLength(chunk);
        if (bytes > (req.url === '/rpc' || req.url === '/local' ? WIRE_BYTES + 8192 : 2048)) throw new Error('Request is too large.');
        data += chunk;
      }
      const input = JSON.parse(data);
      if (!input || typeof input !== 'object' || Array.isArray(input)) throw new Error('Request must be an object.');
      if (req.url === '/local') {
        if (Object.keys(input).some(key => !['action', 'enabled', 'peer', 'route', 'request', 'result', 'address', 'port', 'pairId', 'taskId', 'cleanup', 'discardUncollected', 'harness', 'allModels', 'allowedModels', 'inputBytes', 'inputFiles', 'resultBytes', 'resultFiles', 'retentionDays', 'maxConcurrent', 'keepSessionVisible', 'expiresAt'].includes(key))) throw new Error('Unknown local management field.');
        ...
        if (!matches(token, config.provider?.pairings?.[pairId]?.tokenHash)) throw Object.assign(new Error('Caller authorization was revoked.'), { code: 'PAIRING_REVOKED' });
        ...
        if (nameHeader !== undefined) {
          ...
          if (config.provider.pairings[pairId].name !== callerName) await this.store.update(current => {
            if (!matches(token, current.provider?.pairings?.[pairId]?.tokenHash)) throw Object.assign(new Error('Caller authorization was revoked.'), { code: 'PAIRING_REVOKED' });
        ...
        if (input.action === 'network') {
          if (!crossNetwork(config).enabled) throw new Error('Cross-network connection is off on the provider. Ask its owner to enable it explicitly.');
          ...
          if (['run', 'restore'].includes(input.action)) {
            ...
            if (this.active.size >= maxConcurrent(config.provider) || this.switching) throw new Error('Provider is busy: node capacity is full. Wait for a task to finish before retrying.');
            ...
            if (this.active.has(key)) throw new Error('This task already has an active turn. Query its status before continuing.');
            if (!this.accepting || this.active.size >= maxConcurrent(current.provider) || this.switching || this.harness !== harness) throw new Error('Provider stopped, became busy or changed execution tools during model discovery. Query its current capabilities before retrying.');
            if (!matches(token, current.provider?.pairings?.[pairId]?.tokenHash)) throw Object.assign(new Error('Caller authorization was revoked.'), { code: 'PAIRING_REVOKED' });
            this.requirePairingCapacity(current, pairId);
            const controller = new AbortController();
            const active = { taskId: input.taskId, pairId, controller, response: res, viaTailcat };
            this.active.set(key, active);
            const running = providerRequest({ ...input, harness, ...settings }, { root, executable: this.executable, modelInfo: catalog.find(model => model.model === settings.model), limits: transferLimits(current.provider), retention: retentionDays(current.provider), keepSessionVisible: keepSessionVisible(current.provider), signal: controller.signal, onProgress: progress => send({ progress }) });
            active.done = running;
    ...
    finally { res.end(); this.networkFlows.delete(res); if (networkProtected) { if (res.writableFinished || res.destroyed) this.networkRequests--; else res.once('close', () => { this.networkRequests--; }); } }
  }
  async pairings() {
    const config = await this.store.read();
