...
import { deviceName, codexExecutable, executionSettings, retentionDays, TASK_FILE_SCOPE } from './config.mjs';
...
export class Sharing {
  ...
  async manage(action, input = {}, signal) {
    ...
    const control = ['start', 'pair', 'stop'].includes(action);
    const sequence = control ? ++this.controlSequence : undefined;
    if (control) this.startRequested = action !== 'stop';
