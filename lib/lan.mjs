...
import { deviceName, codexExecutable, executionExecutable, executionHarness, harnessSettings, allowedModels, executionSettings, retentionDays, maxConcurrent, keepSessionVisible, TASK_FILE_SCOPE } from './config.mjs';
...
export class Sharing {
  ...
  async pairings() {
    const config = await this.store.read();
    return { pairings: Object.entries(config.provider?.pairings || {}).map(([pairId, peer]) => ({ pairId, name: peer.name, pairedAt: peer.pairedAt, ...pairingRules(peer) })) };
  }
