...
import { deviceName, codexExecutable, executionExecutable, executionHarness, harnessSettings, allowedModels, executionSettings, retentionDays, maxConcurrent, TASK_FILE_SCOPE } from './config.mjs';
...
export class Sharing {
  ...
  async configure(input, signal) {
    ...
    const settingsFor = provider => ({
      ...
      maxConcurrent: maxConcurrent({ ...provider, ...input }), ...transferLimits({ ...provider, ...input }), retentionDays: retentionDays({ ...provider, ...input })
    });
    ...
    try {
      ...
      if (Object.keys(input).length) await this.store.update(current => {
        ...
        Object.assign(current.provider, transferLimits(settings), { retentionDays: settings.retentionDays, maxConcurrent: settings.maxConcurrent });
      });
      if (switching && this.server) { this.harness = harness; this.executable = executable; }
      this.limit = settings.maxConcurrent;
      return { role: 'provider', maxConcurrent: settings.maxConcurrent, harness, allowedModels: settings.allowedModels, advanced: { ...transferLimits(settings), retentionDays: settings.retentionDays }, note: 'One execution tool is offered at a time. All models includes future available models. Existing tasks retain their original tool and settings.' };
  ...
  async handle(req, res) {
    ...
    try {
      ...
      if (req.url === '/local') {
        if (Object.keys(input).some(key => !['action', 'address', 'port', 'pairId', 'taskId', 'cleanup', 'discardUncollected', 'harness', 'allModels', 'allowedModels', 'inputBytes', 'inputFiles', 'resultBytes', 'resultFiles', 'retentionDays', 'maxConcurrent'].includes(key))) throw new Error('Unknown local management field.');
        ...
        if (input.action === 'usage') {
          ...
          send({ result: { protocol: 2, usage: true, limits: transferLimits(config.provider), transport: 'lan', occupiedSlots: this.active.size, maxConcurrent: maxConcurrent(config.provider), status: !this.accepting ? 'stopped' : this.active.size >= maxConcurrent(config.provider) ? 'busy' : 'available', name: deviceName(config), harness: this.harness, ...executionSettings(harnessSettings(config.provider, this.harness), this.harness) } });
        } else {
          ...
          if (['run', 'restore'].includes(input.action)) {
            ...
            if (input.action === 'status') {
              result.executionActive = this.active.has(`${pairId}/${input.taskId}`);
