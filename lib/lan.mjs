...
export class Sharing {
  ...
  async handle(req, res) {
    ...
    try {
      ...
      if (req.url === '/local') {
        if (Object.keys(input).some(key => !['action', 'address', 'port', 'pairId', 'taskId', 'cleanup', 'discardUncollected', 'harness', 'allModels', 'allowedModels', 'inputBytes', 'inputFiles', 'resultBytes', 'resultFiles', 'retentionDays', 'maxConcurrent', 'keepSessionVisible'].includes(key))) throw new Error('Unknown local management field.');
  ...
  async pairings() {
    ...
    return { pairings: Object.entries(config.provider?.pairings || {}).map(([pairId, peer]) => ({ pairId, name: peer.displayName, deviceName: peer.name, pairedAt: peer.pairedAt })) };
  }
