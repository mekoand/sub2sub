import { STREAM_TYPE, writeMessage, readMessages, readRequest, releaseReceived, retainReceived, parseLegacy, legacyMessage, encodeBody, decodeBody } from './transfer.mjs';
...
import { deviceName, connectionNames, codexExecutable, executionExecutable, executionHarness, harnessSettings, allowedModels, executionSettings, retentionDays, maxConcurrent, keepSessionVisible, cleanupAllOnExpiry, taskRetentionPolicy, requireRetentionConsent, TASK_FILE_SCOPE } from './config.mjs';
...
export function lanRequest(peer, route, input, onProgress = () => {}, signal, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // The local relay hop stays uncompressed. Only the remote hop benefits from gzip.
    const gzip = streaming && route === '/rpc' && peer.transferCompression === 'gzip' && (input.snapshot || input.upload?.files.length);
    ...
    request = https.request({
      ...
      headers: { 'content-type': streaming ? STREAM_TYPE : 'application/json', ...(streaming && route === '/rpc' ? { 'accept-encoding': 'gzip' } : {}), ...(gzip ? { 'content-encoding': 'gzip' } : {}), ...(streaming ? {} : { 'content-length': Buffer.byteLength(body) }), ...(peer.localDeviceName ? { 'x-sub2sub-device-name': encodeURIComponent(peer.localDeviceName) } : {}), ...(peer.token ? { authorization: `Bearer ${peer.token}` } : {}) }
    }, response => {
      if (response.headers['content-type'] === STREAM_TYPE) {
        void (async () => {
          let result, received;
          try {
            for await (const message of readMessages(decodeBody(response, response.headers['content-encoding']), options.receiveLimits ?? ((input.action === 'result' ? input : input.request)?.action === 'result' ? fileLimits((input.action === 'result' ? input : input.request).limits, 'result') : {}))) {
              resetTimer();
      ...
      if (response.headers['content-encoding'] && response.headers['content-encoding'] !== 'identity') { done(new Error('Compressed responses require the streaming transfer protocol.')); return; }
    ...
    request.on('socket', socket => socket.once('secureConnect', () => {
      try {
        ...
        sent = true;
        if (streaming) {
          const output = gzip ? encodeBody(request) : request;
          void writeMessage(output, input).then(() => output.end(), error => done(error));
...
export async function pairPeer(store, input, signal, request = lanRequest) {
  ...
  return { peer: peerName, provider: paired.name, pairId: paired.pairId, transport: 'lan', status: 'paired', harness, model: paired.model, reasoningEffort: paired.reasoningEffort, transferAuthorization, retentionPolicy: paired.retentionPolicy, transferScope: TASK_FILE_SCOPE };
}
...
export class Sharing {
  constructor(store, stateRoot, { independent = false } = {}) {
    ...
    this.controlSequence = 0; this.incomingRequests = 0;
  ...
  async manage(action, input = {}, signal, onProgress) {
    ...
    if (control) this.startRequested = ['start', 'pair'].includes(action) && input.paused !== true;
    ...
    if (action === 'exit') {
      if (this.active.size || this.incomingRequests || this.networkRequests || this.pendingNetwork.size || this.tailcatStarting || this.promoting) throw new Error('An active turn or transfer must finish or be explicitly cancelled before exiting the node.');
      await this.close();
      return this.status();
    }
  ...
  async launch(input, { executable = process.execPath, entry = fileURLToPath(new URL('../bin/share.mjs', import.meta.url)) } = {}) {
    await fs.mkdir(this.root, { recursive: true, mode: 0o700 });
    if (process.platform === 'win32') return launchWindowsNode(entry, this.root, { configFile: path.resolve(this.store.file), stateRoot: path.dirname(this.root), input }, executable);
    const log = await fs.open(path.join(this.root, 'node.log'), 'a', 0o600);
    try {
      const child = spawn(executable, [...(executable === process.execPath ? process.execArgv : []), entry, path.resolve(this.store.file), path.dirname(this.root), JSON.stringify(input)], {
  ...
  async configure(input, signal) {
    ...
    const settingsFor = provider => ({
      ...
      keepSessionVisible: keepSessionVisible({ ...provider, ...input }), maxConcurrent: maxConcurrent({ ...provider, ...input }), ...transferLimits({ ...provider, ...input }), cleanupAllOnExpiry: cleanupAllOnExpiry({ ...provider, ...input }), retentionDays: retentionDays({ ...provider, ...input })
    });
  ...
  async handle(req, res, viaTailcat = false) {
    ...
    const gzip = streaming && req.headers['accept-encoding'] === 'gzip';
      ...
      sending = sending.then(async () => {
        ...
        try {
          if (streaming) {
            ...
            if (gzip) output.flush(zlibConstants.Z_SYNC_FLUSH); // Progress must arrive before the next model event.
          }
    ...
    try {
      ...
      if (res.destroyed) return; // The caller can disconnect while authentication reads configuration.
      if (pairId || req.url === '/local') {
        this.incomingRequests++;
        res.once('close', () => { this.incomingRequests--; });
      }
      ...
      if (!streaming && req.headers['content-encoding'] && req.headers['content-encoding'] !== 'identity') throw new Error('Compression requires the streaming transfer protocol.');
      received = await readRequest(req, { streaming, encoding: req.headers['content-encoding'], ...(req.url === '/rpc' ? fileLimits(config.provider, 'input') : {}), legacyBytes: req.url === '/pair' ? 2048 : WIRE_BYTES + 8192 });
      const input = received.value;
      req.setTimeout(0);
      if (!input || typeof input !== 'object' || Array.isArray(input)) throw new Error('Request must be an object.');
      if (req.url === '/local') {
        if (Object.keys(input).some(key => !['action', 'enabled', 'peer', 'route', 'request', 'result', 'address', 'port', 'pairId', 'taskId', 'cleanup', 'discardUncollected', 'harness', 'allModels', 'allowedModels', 'inputBytes', 'inputFiles', 'resultBytes', 'resultFiles', 'retentionDays', 'cleanupAllOnExpiry', 'maxConcurrent', 'keepSessionVisible', 'expiresAt', 'tokenLimit', 'acceptUsageGap'].includes(key))) throw new Error('Unknown local management field.');
        if (input.action === 'exit') {
          // This authenticated exit request counts once; every other request must finish.
          if (this.active.size || this.incomingRequests > 1 || this.networkRequests || this.pendingNetwork.size || this.tailcatStarting || this.promoting) throw new Error('An active turn or transfer must finish or be explicitly cancelled before exiting the node.');
          this.closing = true;
          this.stop();
        ...
        send({ result: { pairId: id, token: newToken, name: deviceName(config), harness: this.harness, retentionPolicy: taskRetentionPolicy(config.provider), ...executionSettings(harnessSettings(config.provider, this.harness), this.harness) } });
      } else {
        ...
        if (input.action === 'network') {
          ...
          if (!matches(token, config.provider?.pairings?.[pairId]?.tokenHash)) throw Object.assign(new Error('Client authorization was revoked.'), { code: 'PAIRING_REVOKED' });
          send({ result: { ...modelCapabilities(models, config.provider, harness), transferCompression: 'gzip', retentionPolicy: taskRetentionPolicy(config.provider), connection: pairingRules(config.provider.pairings[pairId]) } });
          ...
          if (['run', 'restore'].includes(input.action)) {
            ...
            if (input.action === 'run' && input.snapshot) requireRetentionConsent({ transferAuthorization: { retentionPolicy: input.retentionPolicy } }, taskRetentionPolicy(current.provider));
            ...
            const running = providerRequest({ ...input, harness, ...settings }, {
              ...
              root, executable: this.executable, modelInfo: catalog.find(model => model.model === settings.model), limits: transferLimits(current.provider), retention: retentionDays(current.provider), cleanupAllOnExpiry: cleanupAllOnExpiry(current.provider), keepSessionVisible: keepSessionVisible(current.provider), signal: controller.signal, onProgress: progress => send({ progress }) });
            active.done = running;
            ...
            const result = await providerRequest(input, { root, executable, limits: transferLimits(config.provider), onProgress: progress => send({ progress }), ...(input.action === 'result' ? { onResult: async packet => { send({ result: packet }); await sending; return packet; } } : {}) });
            ...
            if (input.action !== 'result') send({ result });
          }
        }
      }
    } catch (error) { send({ error: error.message, ...(error.code === 'PAIRING_REVOKED' ? { code: error.code } : {}) }); }
    finally { await sending; await releaseReceived(received?.value); if (!sendError) output.end(); this.networkFlows.delete(res); if (networkProtected) { if (res.writableFinished || res.destroyed) this.networkRequests--; else res.once('close', () => { this.networkRequests--; }); } }
  }
  async pairings() {
    const config = await this.store.read();
  ...
  async cleanup(pairId, cleanup, signal) {
    ...
    for (const id of files.filter(name => !name.startsWith('.'))) {
      ...
      await providerRequest({ action: 'finish', taskId: id, cleanup }, { root, executable, signal, ownerCleanup: true });
    }
  ...
  async cleanupExpired() {
    ...
    const expired = tasks.filter(t => t.expired && t.status !== 'expired' && (t.cleanupAllOnExpiry || t.status !== 'released'));
    const config = await this.store.read();
    ...
    for (const task of expired) {
      ...
      if (this.active.has(`${task.pairId}/${task.taskId}`) || ['running', 'unknown', 'restoring'].includes(task.status) || task.status === 'finishing' && !task.cleanupAllOnExpiry) report.reason = `Task state ${task.status} does not permit automatic cleanup.`;
      else if (!task.cleanupAllOnExpiry && (!task.saveConfirmed || task.savedRevision !== task.revision || task.unresolvedSkipped?.length)) report.reason = 'Waiting for confirmation that all necessary results are saved locally.';
      else {
        try {
          await providerRequest({ action: 'finish', taskId: task.taskId, cleanup: task.cleanupAllOnExpiry ? 'all' : 'workcopy', revision: task.savedRevision }, { root: path.join(this.root, 'tasks', task.pairId), executable: task.cleanupAllOnExpiry && executionHarness(task) === 'codex' ? await codexExecutable(config) : undefined, ownerCleanup: task.cleanupAllOnExpiry === true, expiryCleanup: task.cleanupAllOnExpiry === true });
