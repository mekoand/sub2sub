...
export async function pairPeer(store, input, signal) {
  ...
  await store.update(current => {
    ...
    current.peers[peerName] = { deviceName: paired.name, automaticName: input.peer === undefined, transport: 'lan', host: invite.host, port: invite.port, fingerprint: invite.fingerprint, pairId: paired.pairId, token: paired.token, transferAuthorization };
...
export class Sharing {
  ...
  async manage(action, input = {}, signal) {
    ...
    if (action === 'exit') {
      if (this.active.size) throw new Error('An active turn must finish or be explicitly cancelled before exiting the node.');
      await this.close();
      return this.status();
    }
  ...
  async handle(req, res) {
    ...
    try {
      ...
      if (req.url === '/local') {
        if (Object.keys(input).some(key => !['action', 'address', 'port', 'pairId', 'taskId', 'cleanup', 'discardUncollected', 'harness', 'allModels', 'allowedModels', 'inputBytes', 'inputFiles', 'resultBytes', 'resultFiles', 'retentionDays', 'maxConcurrent', 'keepSessionVisible', 'expiresAt'].includes(key))) throw new Error('Unknown local management field.');
        ...
        if (input.action === 'usage') {
          ...
          if (['run', 'restore'].includes(input.action)) {
            ...
            const active = { taskId: input.taskId, pairId, controller, response: res };
