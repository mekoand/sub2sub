...
export async function processLock(directory) {
  ...
  try {
    ...
    try { await fs.rename(candidate, directory); }
      ...
      try { process.kill(owner.pid, 0); throw Object.assign(new Error(`Another sub2sub process (${owner.pid}) owns ${directory}. Retry after its current operation finishes.`), { code: 'LOCK_BUSY' }); }
