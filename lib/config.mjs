...
export const TASK_FILE_SCOPE = 'Send the files and instructions needed for tasks you delegate to this device, including non-public project source, documents and necessary configuration. The host can access these materials. Excludes credentials, unrelated files and sensitive material needing separate consent. Work copies follow task retention and saved-result cleanup conditions; ordinary cleanup retains native history. Withdrawal stops future transfers; neither withdrawal nor cleanup recalls copies or backups the host already made. This does not override managing-app approvals.';
export function crossNetwork(config) {
  const enabled = config.crossNetwork?.enabled ?? false;
  if (typeof enabled !== 'boolean') throw new Error('crossNetwork.enabled must be a boolean.');
  return { enabled };
}
...
export function hostSelection(caller = {}) {
  const autoSelectHost = caller.autoSelectHost ?? false;
  const candidateHosts = caller.candidateHosts ?? [];
  if (typeof autoSelectHost !== 'boolean') throw new Error('autoSelectHost must be a boolean.');
  if (!Array.isArray(candidateHosts) || candidateHosts.length > 2000 || candidateHosts.some(name => typeof name !== 'string' || !name.trim() || name.length > 100) || new Set(candidateHosts).size !== candidateHosts.length) throw new Error('candidateHosts must be an ordered list of unique connection names.');
  return { autoSelectHost, candidateHosts };
}
...
export function cleanupAllOnExpiry(provider = {}) {
  const value = provider.cleanupAllOnExpiry ?? false;
  if (typeof value !== 'boolean') throw new Error('cleanupAllOnExpiry must be a boolean.');
  return value;
}

export function taskRetentionPolicy(provider = {}) {
  return cleanupAllOnExpiry(provider) ? { cleanupAllOnExpiry: true, retentionDays: retentionDays(provider) } : undefined;
}

export function requireRetentionConsent(peer, policy) {
  const accepted = peer.transferAuthorization?.retentionPolicy;
  if (!policy) {
    if (accepted) throw new Error('Retention consent no longer matches: the host no longer offers automatic deletion of all task content. Review its current rule before renewing file-transfer consent.');
    return;
  }
  if (accepted?.cleanupAllOnExpiry !== true || accepted.retentionDays !== policy.retentionDays) {
    throw new Error(`Retention consent required: after ${policy.retentionDays} idle days the host deletes all task content and associated native history, including uncollected results. The original session cannot continue. Explain this rule and use authorize_peer with retentionPolicy after consent.`);
  }
}
...
export class Config {
  ...
  async read() {
    ...
    if (config.crossNetwork !== undefined && (!config.crossNetwork || typeof config.crossNetwork !== 'object' || Array.isArray(config.crossNetwork) || typeof config.crossNetwork.enabled !== 'boolean')) throw new Error('crossNetwork.enabled must be a boolean.');
    return config;
  }
  update(change) {
    const write = async () => {
