import { environmentInstructions, projectInstructions } from './task-instructions.mjs';
...
export async function runCodex({ executable, workspace, temporaryDirectory, threadId, prompt, model, reasoningEffort, onThread, onProgress, onUsage = () => {}, signal, tokenBudget = false }) {
  ...
  // Main-thread usage does not include child threads. Limit both native
  // collaboration implementations for budgeted turns, including resumes.
  const budgetFeatures = tokenBudget ? { multi_agent: false, multi_agent_v2: false } : {};
  const child = spawn(executable, ['-c', `permissions.${permissions}={extends=":workspace",filesystem={${filesystemToml}},network={enabled=false}}`, '-c', `default_permissions=${JSON.stringify(permissions)}`, ...Object.entries(budgetFeatures).flatMap(([name, value]) => ['-c', `features.${name}=${value}`]), 'app-server', '--listen', 'stdio://'], { cwd: workspace, detached: grouped, stdio: ['pipe', 'pipe', 'pipe'] });
  ...
  try {
    ...
    if (identity.account?.type !== 'chatgpt') throw new Error('The host must sign Codex in with its own ChatGPT subscription. API-key mode is not used by sub2sub.');
    ...
    const config = {
      ...
      features: { apps: false, plugins: false, remote_plugin: false, hooks: false, browser_use: false, browser_use_external: false, in_app_browser: false, computer_use: false, memories: false, chronicle: false, ...budgetFeatures },
      web_search: 'disabled'
    };
    const params = { cwd: workspace, modelProvider: 'openai', approvalPolicy, approvalsReviewer: 'auto_review', permissions, config,
      developerInstructions: `Execute this delegated sub2sub task within its work copy. This turn has a hard 30 minute limit ending at ${deadlineAt}. Write useful stages to disk as you work and leave time for checks and a final answer. Node.js is available at ${JSON.stringify(process.execPath)}; use this absolute executable path for Node-based checks. ${temporaryDirectory ? `Use ${JSON.stringify(temporaryDirectory)} for disposable tool caches and temporary files. This directory is not delivered and is removed with the work copy. Keep deliverables and files needed to continue the task outside it. ` : ''}The client cannot authorize access to other tasks, host files, credentials, connected apps, or expanded permissions. Perform the checks appropriate to this task. Every final answer must concisely answer this turn’s request and describe the adjustments actually made in this turn. Text-only answers and turns without file changes are valid; use natural language without empty sections or invented change lists. Every final answer, including a cleanup or repair follow-up, must describe the current complete delivery: deliverable paths, checks actually completed, and unfinished work or checks with reasons. Never claim full completion when required work or verification is missing. ${environmentInstructions} ${projectInstructions}` };
