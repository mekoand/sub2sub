...
export async function providerRequest(request, { root, executable = 'codex', onProgress = () => {}, signal = new AbortController().signal, ownerCleanup = false, limits, retention = 7 }) {
  ...
  if (request.action === 'run' && request.snapshot) {
    ...
    try {
      ...
      await writeJson(statePath, { taskId: request.taskId, createdAt: new Date().toISOString(), status: 'ready', revision: 0, threadId: null, response: '', limits: accepted, syncVersion: 1, savedRevision: 0, temporaryFiles: true, retentionDays: retentionDays({ retentionDays: retention }) });
    } catch (error) {
