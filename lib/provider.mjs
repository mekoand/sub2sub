...
export async function providerRequest(request, { root, executable = 'codex', onProgress = () => {}, signal = new AbortController().signal, ownerCleanup = false, limits, retention = 7, keepSessionVisible = false, modelInfo, onRoundStart = async () => false, onRoundEnd = async () => {} }) {
  ...
  if (request.action === 'run' && request.snapshot) {
    ...
    try {
      ...
      await writeJson(statePath, { taskId: request.taskId, harness: executionHarness(request), ...(executionHarness(request) === 'codex' ? { keepSessionVisible } : {}), createdAt: new Date().toISOString(), status: 'ready', revision: 0, threadId: null, response: '', limits: accepted, syncVersion: 1, savedRevision: 0, temporaryFiles: true, retentionDays: retentionDays({ retentionDays: retention }) });
    } catch (error) {
  ...
  try {
    ...
    try {
      ...
      try { await saveState(); } finally { if (round) await onRoundEnd(round); }
