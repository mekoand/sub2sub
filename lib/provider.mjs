...
import { executionSettings, retentionDays } from './config.mjs';
...
export async function providerRequest(request, { root, executable = 'codex', onProgress = () => {}, signal = new AbortController().signal, ownerCleanup = false, limits, retention = 7 }) {
  ...
  if (request.action === 'run' && request.snapshot) {
    ...
    try {
      ...
      await writeJson(statePath, { taskId: request.taskId, createdAt: new Date().toISOString(), status: 'ready', revision: 0, threadId: null, response: '', limits: accepted, syncVersion: 1, savedRevision: 0, retentionDays: retentionDays({ retentionDays: retention }) });
    } catch (error) {
      await writeJson(statePath, { taskId: request.taskId, status: 'failed', revision: 0, threadId: null, response: '', error: error.message });
      throw error;
    }
  }
  ...
  try {
    ...
    if (request.action === 'finish') {
      ...
      if (request.cleanup === 'workcopy') {
        ...
        if (!['releasing', 'released'].includes(state.status)) {
          if (!ownerCleanup) {
            ...
            const delta = await collectChanges(path.join(directory, 'work'), await readJson(path.join(directory, 'original.json')), fileLimits(state.limits, 'result'));
      ...
      const history = request.cleanup === 'all'
        ? await manageTaskHistory({ executable, root, taskId: request.taskId, threadId: state?.threadId, signal })
        : state?.threadId && !state.historyTagged ? await manageTaskHistory({ executable, root, taskId: request.taskId, threadId: state.threadId, retain: true, signal }) : {};
    ...
    if (request.action === 'restore') {
      ...
      if ((await inspectTask(directory)).workCopyExists) {
        const delta = await collectChanges(work, request.snapshot.files, fileLimits(state.limits, 'result'));
    ...
    if (request.action === 'result') {
      ...
      const { manifest, ...changes } = await collectChanges(path.join(directory, 'work'), await readJson(path.join(directory, 'original.json')), fileLimits(state.limits, 'result'));
    ...
    try {
      ...
      state.response = '';
      state.progress = '';
      const settings = executionSettings(request);
      Object.assign(state, settings);
      await writeJson(statePath, state);
      const result = await runCodex({
        executable, workspace: path.join(directory, 'work'), threadId: state.threadId,
        prompt: request.prompt, ...settings, signal: controller.signal,
