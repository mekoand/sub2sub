...
import { applyUpload, recoverUpload, manifestDigest } from './upload.mjs';
...
export async function providerRequest(request, { root, executable = 'codex', onProgress = () => {}, signal = new AbortController().signal, ownerCleanup = false, expiryCleanup = false, limits, retention = 7, cleanupAllOnExpiry = false, keepSessionVisible = false, modelInfo, onResult, onRoundStart = async () => false, onRoundEnd = async () => {} }) {
  ...
  if (request.restoreOnly !== undefined && (request.action !== 'restore' || request.restoreOnly !== true)) throw new Error('restoreOnly is only valid for work copy restoration.');
  ...
  let release, created = false;
  try {
    await fs.mkdir(lock); created = true;
    await writeJson(path.join(lock, `${randomUUID()}.json`), { pid: process.pid });
    release = () => fs.rm(lock, { recursive: true });
  } catch (error) {
    if (created) { await fs.rm(lock, { recursive: true }); throw error; }
    if (error.code !== 'EEXIST') throw error;
    // Legacy empty locks have no inspectable owner; never replace one blindly.
    if (!(await fs.readdir(lock)).length) throw new Error('This task has a legacy running lock. The host must check the previous process before removing it.');
    try { release = await processLock(lock); }
    catch (error) { if (error.code === 'LOCK_BUSY') throw new Error('This task already has a running turn.', { cause: error }); throw error; }
  }
  try {
    ...
    if (state?.status === 'updating') {
      await recoverUpload(directory, state);
    ...
    if (request.action === 'restore') {
      ...
      if ((request.upload || request.restoreOnly) && (!state.savedManifestDigest || manifestDigest(request.snapshot.files) !== state.savedManifestDigest)) throw new Error('The recovery copy differs from the confirmed input baseline. No new turn started.');
      ...
      await writeJson(statePath, state);
      if (request.restoreOnly) return state;
    ...
    if (request.action === 'ack') {
      ...
      const manifest = await readJson(path.join(directory, 'pending-manifest.json'));
      await writeJson(path.join(directory, 'original.json'), manifest);
      state.savedManifestDigest = manifestDigest(manifest);
    ...
    if (request.upload) {
      const accepted = effectiveLimits(state.limits ?? legacyLimits(), effectiveLimits(request.limits ?? (request.protocol === 3 ? {} : legacyLimits()), limits));
      await applyUpload(directory, state, request.upload, fileLimits(accepted, 'input'));
    }
