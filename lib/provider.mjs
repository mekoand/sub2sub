...
export async function providerRequest(request, { root, executable = 'codex', onProgress = () => {}, signal = new AbortController().signal, ownerCleanup = false, limits, retention = 7, keepSessionVisible = false, modelInfo }) {
  ...
  try {
    ...
    try {
      ...
      // Persist the stopped turn before native presentation maintenance can wait or fail.
      await saveState();
