...
// WMI starts in the same user's session, outside the managing app's job.
// A normal detached child is still reaped with Windows SSH and some GUI hosts.
export async function launchWindowsNode(entry, root, options, executable = process.execPath) {
  const startupFile = path.join(root, `startup-${randomUUID()}.json`);
  await writeJson(startupFile, options);
  let pid;
  try {
    const command = [executable, entry, '--startup-file', startupFile].map(arg => `"${arg}"`).join(' ');
...
Write-Output $result.ProcessId`;
    ...
    for (;;) {
      ...
      if (['sharing', 'stopped'].includes(startup.result?.status)) return;
