...
export async function updatePlugin(client, input = {}, signal) {
  ...
  } catch (error) { throw new Error(`Release download/installation failed: ${(error.stderr || error.message).slice(-4000)} ${(error.stdout || '').slice(-1000)}. Existing sessions and saved data were not stopped or cleaned. Check managing-app registration before retrying; an interrupted installer may have partially installed the release.`, { cause: error }); }
  const verified = await registration(client, installed.host);
  if (verified.status !== 'registered' || !verified.enabled || verified.version.replace(/\+(codex|claude)\.\d+$/, '') !== latest.version) throw new Error(`Installation verification failed: ${verified.reason || 'the managing app does not report the requested version as enabled'}. Check the managing app plugin list before retrying. The running session and node still use their existing code.`);
  return { ...result, status: 'installed', installed: verified, nextStep: `${installed.host === 'codex' ? 'Fully quit and reopen Codex Desktop to load the updated plugin; closing a window or starting a new conversation is not sufficient. For Codex CLI, exit and restart the CLI session.' : 'Exit and restart Claude Code to load the updated plugin.'} Existing independent nodes keep running: when idle, use exit_sharing then start_sharing from the restarted managing app. For 0.5.3 sharing, finish/save work and end the old host conversation before starting sharing again; that old version has no independent-node exit command. Peer features still depend on existing capability checks; mixed versions are not universally compatible.` };
}
