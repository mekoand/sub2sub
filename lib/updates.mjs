...
import { maintainInstallation } from './installation.mjs';
...
async function registration(client, host) {
  if (!['codex', 'claude'].includes(host)) return { status: 'unknown', host: null, version: null, root: null, reason: 'The managing app is unknown. Specify host=codex or host=claude; no installation directory is guessed.' };
  try {
    ...
    if (!root || !path.isAbsolute(root) || !installed?.version) throw new Error('No registered sub2sub release installation was found. Use the documented installer to establish this app installation.');
    if (process.env.SUB2SUB_INSTALL_DIR && path.resolve(process.env.SUB2SUB_INSTALL_DIR) !== path.resolve(root)) throw new Error('The managing-app registration differs from this session installation directory. Open a new session in the intended app before upgrading.');
...
export async function updatePlugin(client, input = {}, signal) {
  ...
  if (input.host && process.env.SUB2SUB_INSTALL_HOST && input.host !== process.env.SUB2SUB_INSTALL_HOST) throw new Error('The requested app differs from the current managing app. Open a session in that app to upgrade it.');
  const installed = await registration(client, input.host || process.env.SUB2SUB_INSTALL_HOST);
  const node = await nodeStatus(client);
  let latest;
  try {
  ...
  const cleanup = client.programCleanup ? await client.programCleanup : undefined;
  const result = { status, ...(cleanup ? { cleanup } : {}), sessionVersion: packageInfo.version, installed, node, latest, checkedAt: new Date().toISOString() };
  if (input.action === 'check' || status === 'newer_installed') return result;
  ...
  try {
    ...
    if (busy) return { ...result, status: 'deferred', reason: `Host task ${busy.taskId} is ${busy.status}; resolve its work before upgrading.` };
    const locks = await fs.readdir(path.join(client.stateRoot, 'tasks/.locks')).catch(error => { if (error.code === 'ENOENT') return []; throw error; });
    if (locks.length) return { ...result, status: 'deferred', reason: 'A client task operation is active or its lock has not been resolved. Wait for it to finish before upgrading.' };
    ...
    for (const file of files.filter(name => name.endsWith('.json'))) {
      ...
      if (!task.finished && task.deliveryPending !== false) return { ...result, status: 'deferred', reason: `Local client task ${file.slice(0, -5)} has active or unsaved/unknown work. Check task_status and collect_result before upgrading.` };
    }
  ...
  if (status === 'current') {
    const maintenance = await maintainInstallation(client.sharing, installed.root, installedBase);
    return { ...result, node: await nodeStatus(client), maintenance, nextStep: restartNote(installed.host) };
  }
  ...
  let installation;
  try {
    const { stdout } = await exec(command, args, { env: { ...process.env, SUB2SUB_VERSION: latest.version, SUB2SUB_INSTALL_DIR: installed.root }, windowsHide: true, maxBuffer: 8 * 1024 * 1024 });
    const report = stdout.split(/\r?\n/).findLast(line => line.startsWith('sub2sub-install-result: '));
    if (report) installation = JSON.parse(report.slice('sub2sub-install-result: '.length));
  } catch (error) { throw new Error(`Release download/installation failed: ${(error.stderr || error.message).slice(-4000)} ${(error.stdout || '').slice(-1000)}. An interrupted installer may have changed registration or switched an idle node. Check the managing app plugin list and sharing_status before retrying; the running node state is not confirmed by this error.`, { cause: error }); }
  const verified = await registration(client, installed.host);
  if (verified.status !== 'registered' || !verified.enabled || verified.version.replace(/\+(codex|claude)\.\d+$/, '') !== latest.version) throw new Error(`Installation verification failed: ${verified.reason || 'the managing app does not report the requested version as enabled'}. Check the managing app plugin list and sharing_status before retrying. The current conversation still uses its loaded code, but the node may already have switched. Maintenance report: ${JSON.stringify(installation?.maintenance || null)}.`);
  const maintenance = installation?.maintenance || await maintainInstallation(client.sharing, installed.root, latest.version);
  return { ...result, status: 'installed', installed: verified, node: await nodeStatus(client), maintenance, nextStep: restartNote(installed.host) };
}

function restartNote(host) {
  return `${host === 'codex' ? 'Fully quit and reopen Codex Desktop to load the updated plugin; closing a window or starting a new conversation is not sufficient. For Codex CLI, exit and restart the CLI session.' : 'Exit and restart Claude Code to load the updated plugin.'} Check maintenance.node for the independent node switch and maintenance.cleanup for removed, retained or failed old programs. When switching is deferred, resolve the stated work or compatibility limit and retry update_plugin(action=install). No user data is migrated or deleted. Peer features still depend on capability checks.`;
}

async function nodeStatus(client) {
  try {
    const state = await client.sharing.machineStatus();
    return { status: state.status, version: state.version || null, activeTask: state.activeTask, ownerPid: state.ownerPid, reason: state.reason };
  } catch (error) { return { status: 'unknown', version: null, reason: error.message }; }
}
