...
export class Client {
  ...
  async taskCall(name, input, onProgress, signal) {
    ...
    let settings, restoration, executionTool, upload;
    if (name === 'continue_task') {
      ...
      if (['released', 'restoring'].includes(remote.status)) {
        ...
        if (upload) {
          ...
          await this.connect(peer, { ...restoration, restoreOnly: true, protocol: 3, taskId: id, prompt: input.prompt, ...settings, ...executionTool }, onProgress, signal);
    ...
    let result;
    try { result = await this.connect(peer, { action: actions[name], protocol: task.transferProtocol || 2, limits: task.limits ?? legacyLimits(), taskId: id, prompt: input.prompt, details: input.details, ...(upload ? { upload } : {}), ...settings, ...executionTool, ...restoration }, onProgress, signal); }
    catch (error) {
