...
export class Client {
  ...
  async taskCall(name, input, onProgress, signal) {
    ...
    let settings, restoration, executionTool, transferCompression;
    ...
    let result;
    try { result = await this.connect({ ...peer, transferCompression }, { action: actions[name], protocol: task.transferProtocol || 2, limits: task.limits ?? legacyLimits(), taskId: id, prompt: input.prompt, details: input.details, ...settings, ...executionTool, ...restoration }, onProgress, signal); }
    catch (error) {
