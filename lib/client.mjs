...
import { Config, executionSettings, executionHarness, harnessSettings, allowedModels, retentionDays, maxConcurrent, keepSessionVisible, deviceName, TASK_FILE_SCOPE } from './config.mjs';
...
export class Client {
  ...
  async capabilities(peer, onProgress, signal) {
    try {
      const result = await connect(peer, { action: 'models' }, onProgress, signal);
  ...
  async onboarding(input = {}) {
    ...
    return {
      ...
      connections: Object.entries(config.peers || {}).map(([name, peer]) => ({ name, transport: peer.transport, host: peer.host, port: peer.port, taskRoot: peer.taskRoot, runtimePath: peer.runtimePath, nodePath: peer.nodePath, codexPath: peer.codexPath, status: 'unchecked', transferAuthorization: { scope: peer.transferAuthorization?.scope || 'none' } })),
  ...
  async call(name, input, onProgress, signal) {
    ...
    if (name === 'list_peers') {
      const peers = await Promise.all(Object.entries(this.config.peers || {}).map(async ([name, peer]) => {
        ...
        try { const current = await connect(peer, { action: 'check' }, onProgress, signal); return { ...summary, status: current.status || 'available', harness: current.harness || 'codex', limits: current.limits, connection: current.connection, checkedAt: new Date().toISOString() }; }
    ...
    if (name === 'start_task') {
      ...
      try {
        ...
        result = await connect(peer, { action: 'run', protocol: 2, taskId: id, snapshot: { files: copy.files }, prompt: input.prompt, ...settings, ...(capabilities.harness ? { harness } : {}), limits }, onProgress, signal);
  ...
  async taskCall(name, input, onProgress, signal) {
    ...
    let result;
    try { result = await connect(peer, { action: actions[name], protocol: 2, taskId: id, prompt: input.prompt, details: input.details, ...settings, ...executionTool, ...restoration }, onProgress, signal); }
    catch (error) {
