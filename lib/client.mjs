...
export class Client {
  ...
  async networkRequest(peer, route, input, onProgress, signal) {
    ...
    return { ...result, ...(peer.tailcat ? { connectionRoute: 'direct' } : {}) };
  }
    ...
    return {
      ...
      settings: {
        ...
        advanced: {
          ...
          supportedLimits: { bytes: SUPPORT_BYTES, files: SUPPORT_FILES }
        }
      },
  ...
  async call(name, input, onProgress, signal) {
    ...
    if (name === 'start_task') {
      ...
      try {
        ...
        result = await this.connect(peer, { action: 'run', protocol: 2, taskId: id, snapshot: { files: copy.files }, prompt: input.prompt, ...settings, ...(capabilities.harness ? { harness } : {}), limits }, onProgress, signal);
  ...
  async taskCall(name, input, onProgress, signal) {
    ...
    if (name === 'finish_task') {
      ...
      const finished = await this.connect(peer, { action: 'finish', ...(task.harness === 'claude' ? { harness: 'claude' } : {}), taskId: id, revision: task.resultRevision, discardPaths: input.discardPaths, cleanup: input.cleanup }, onProgress, signal);
    ...
    let result;
    try { result = await this.connect(peer, { action: actions[name], protocol: 2, taskId: id, prompt: input.prompt, details: input.details, ...settings, ...executionTool, ...restoration }, onProgress, signal); }
    catch (error) {
