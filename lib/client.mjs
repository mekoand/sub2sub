...
import { Config, executionSettings, executionHarness, harnessSettings, allowedModels, retentionDays, maxConcurrent, keepSessionVisible, deviceName, connectionNames, cleanupAllOnExpiry, requireRetentionConsent, TASK_FILE_SCOPE } from './config.mjs';
  ...
  async call(name, input, onProgress, signal) {
    ...
    if (name === 'start_task') {
      ...
      try {
        ...
        result = await this.connect(peer, { action: 'run', protocol: transferProtocol, taskId: id, snapshot: upload, prompt: input.prompt, ...(capabilities.retentionPolicy ? { retentionPolicy: capabilities.retentionPolicy } : {}), ...settings, ...(capabilities.harness ? { harness } : {}), limits }, onProgress, signal);
