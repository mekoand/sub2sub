...
export class Client {
  ...
  async connect(peer, request, onProgress, signal) {
    const result = await connect({ ...peer, localDeviceName: deviceName(this.config) }, request, onProgress, signal);
  ...
  async capabilities(peer, onProgress, signal) {
    try { return await this.connect(peer, { action: 'models' }, onProgress, signal); }
  ...
  async call(name, input, onProgress, signal) {
    ...
    if (this.store) {
      ...
      if (status === 'pending' && (name === 'caller_settings' && Object.keys(input).some(key => key !== 'harness') || ['provider_settings', 'configure_model'].includes(name) && Object.keys(input).length)) {
    ...
    if (name === 'device_settings') {
      ...
      return { deviceName: deviceName(this.config), note: 'sub2sub display name only. Paired devices learn it on subsequent communication; explicit connection aliases remain unchanged.' };
    }
    ...
    if (name === 'list_peers') {
      const peers = await Promise.all(Object.entries(this.config.peers || {}).map(async ([name, peer]) => {
        const summary = { name, displayName: peer.displayName || name, deviceName: peer.deviceName, transport: peer.transport, transferAuthorization: peer.transferAuthorization || { scope: 'none' } };
        if (input.check === false) return { ...summary, status: 'unchecked' };
        try { const current = await this.connect(peer, { action: 'check' }, onProgress, signal); return { ...summary, displayName: peer.displayName || name, deviceName: peer.deviceName, status: current.status || 'available', harness: current.harness || 'codex', limits: current.limits, checkedAt: new Date().toISOString() }; }
      ...
      const guidance = {
        ...
        busy: 'Busy peers have reached their node capacity; wait for a task to finish before delegating another.',
    ...
    try { return await this.store.update(async config => {
      ...
      if (name === 'delete_peer') {
        ...
        for (const record of records.filter(r => !r.saved.finished)) {
          let current;
          try { current = await connect({ ...peer, localDeviceName: deviceName(config) }, { action: 'status', taskId: record.id }, onProgress, signal); }
          catch (error) {
