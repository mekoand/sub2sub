...
import { snapshot, materialize, readJson, writeJson, validateFiles, safePath } from './files.mjs';
import { taskId } from './provider.mjs';
import { Config, executionSettings, executionHarness, harnessSettings, allowedModels, retentionDays, maxConcurrent, keepSessionVisible, deviceName, connectionNames, TASK_FILE_SCOPE } from './config.mjs';
      ...
      reading = (async () => {
        for await (const message of readMessages(child.stdout, request.action === 'result' ? fileLimits(request.limits ?? legacyLimits(), 'result') : {})) {
          if ('error' in message) failure = new Error(`Peer: ${message.error}`);
            ...
            if (result) { await releaseReceived(message); throw new Error('Duplicate provider result.'); }
...
export class Client {
  ...
  async capabilities(peer, onProgress, signal) {
    try {
      ...
      if (result.connection?.authorizationStatus === 'expired') throw new Error('Connection authorization has expired. Ask the provider to extend the original connection. Existing results remain accessible.');
      return result;
    }
    catch (error) { throw new Error(`Cannot confirm provider capabilities: ${error.message}. The cause is unconfirmed; review this error and query list_models when ready. No task files were sent and no model was selected.`, { cause: error }); }
  }
  async initialize() {
  ...
  async onboarding(input = {}) {
    ...
    return {
      ...
      settings: {
        ...
        provider: {
          role: 'Share this device’s execution environment with authorized callers.',
          ...
          modelAvailability: 'unverified',
          maxConcurrent: maxConcurrent(config.provider), keepSessionVisible: keepSessionVisible(config.provider), limits: transferLimits(config.provider), retentionDays: retentionDays(config.provider)
        },
        advanced: {
  ...
  async call(name, input, onProgress, signal) {
    ...
    if (name === 'caller_settings') {
      ...
      return { role: 'caller', harness, execution: executionSettings(harnessSettings(saved, harness), harness), advanced: transferLimits(saved), supported: { bytes: null, files: null }, note: 'Defaults are shared by nodes using this execution tool. Null means no default has been chosen: list the node models and ask the user to select a model and effort before sending files. Existing tasks keep their own choices.' };
    }
    ...
    if (name === 'list_peers') {
      ...
      const guidance = {
        ...
        expired: 'Ask the provider to extend the original connection. Existing task results remain accessible.',
    ...
    if (name === 'start_task') {
      ...
      catch (error) { if (capabilities.transferProtocol !== 3) throw new Error(`Older provider transfer limits apply. Upgrade both nodes or reduce this work copy: ${error.message}`, { cause: error }); throw error; }
      ...
      const saved = { peer: input.peer, peerId: peer.pairId || peer.id, snapshotId: input.snapshotId, transferProtocol, harness, ...settings, limits, createdAt: new Date().toISOString(), deliveryPending: true };
      let release;
      ...
      try {
        ...
        result = await this.connect(peer, { action: 'run', protocol: transferProtocol, taskId: id, snapshot: upload, prompt: input.prompt, ...settings, ...(capabilities.harness ? { harness } : {}), limits }, onProgress, signal);
        await writeJson(taskPath, { ...saved, status: result.status, revision: result.revision, rounds: measuredRounds(result.rounds), endedAt: result.endedAt, keepSessionVisible: result.keepSessionVisible, sessionVisibility: result.sessionVisibility, retentionDays: result.retentionDays, expiresAt: result.expiresAt });
        ...
        throw new Error(`Task ${id}: ${error.message}${next}`, { cause: error });
      }
  ...
  async taskCall(name, input, onProgress, signal) {
    ...
    let result;
    try { result = await this.connect(peer, { action: actions[name], protocol: task.transferProtocol || 2, limits: task.limits ?? legacyLimits(), taskId: id, prompt: input.prompt, details: input.details, ...settings, ...executionTool, ...restoration }, onProgress, signal); }
    catch (error) {
    ...
    if (name === 'continue_task' || name === 'collect_result') {
      Object.assign(task, { status: result.status, revision: result.revision, rounds: measuredRounds(result.rounds), endedAt: result.endedAt, keepSessionVisible: result.keepSessionVisible, sessionVisibility: result.sessionVisibility, retentionDays: result.retentionDays, expiresAt: result.expiresAt });
      if (name === 'continue_task') {
    ...
    try { return await this.store.update(async config => {
      ...
      if (input.migrate) {
        ...
        if (input.invitation) endpoint = invitation(input.invitation);
          ...
          catch (error) { throw new Error(`Cannot obtain this provider's cross-network address: ${error.message}. Enable the service on the provider and update older nodes. If its old address is unavailable, supply a new invitation from that same device; do not delete or re-pair.`, { cause: error }); }
        }
        if (endpoint?.fingerprint !== peer.fingerprint) throw new Error('The invitation or endpoint belongs to a different provider identity. The original connection was retained.');
        if (typeof endpoint.tailcat !== 'string' || !endpoint.tailcat) throw new Error('This invitation cannot describe cross-network access. Update the provider and enable cross-network connections before creating a new invitation. The original direct connection was retained.');
      ...
      connectionNames(Object.values(config.peers), p => p.automaticName ? p.deviceName : Object.keys(config.peers).find(key => config.peers[key] === p));
      return { peer: nextName, status: 'updated', crossNetwork: Boolean(peer.tailcat), note: 'Task association, provider identity and transfer consent are preserved.' };
    });
