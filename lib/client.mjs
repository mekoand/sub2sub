...
import { Config, executionSettings, executionHarness, harnessSettings, allowedModels, retentionDays, maxConcurrent, deviceName, TASK_FILE_SCOPE } from './config.mjs';
...
export class Client {
    ...
    return {
      ...
      settings: {
        ...
        provider: {
          ...
          modelAvailability: 'unverified',
          maxConcurrent: maxConcurrent(config.provider), limits: transferLimits(config.provider), retentionDays: retentionDays(config.provider)
        },
        advanced: {
  ...
  async call(name, input, onProgress, signal) {
    ...
    if (name === 'start_task') {
      ...
      try {
        ...
        await writeJson(taskPath, { ...saved, status: result.status, revision: result.revision, rounds: measuredRounds(result.rounds), endedAt: result.endedAt, retentionDays: result.retentionDays, expiresAt: result.expiresAt });
  ...
  async taskCall(name, input, onProgress, signal) {
    ...
    if (name === 'continue_task' || name === 'collect_result') {
      Object.assign(task, { status: result.status, revision: result.revision, rounds: measuredRounds(result.rounds), endedAt: result.endedAt, retentionDays: result.retentionDays, expiresAt: result.expiresAt });
      if (name === 'continue_task') {
