...
import { Config, executionSettings, allowedModels, retentionDays, TASK_FILE_SCOPE } from './config.mjs';
import { Sharing, pairPeer, lanRequest } from './lan.mjs';
import { queryModels, modelCapabilities, requireModel } from './models.mjs';
import { codexExecutable } from './config.mjs';
...
export class Client {
  constructor(config, stateRoot, store) {
    ...
    if (store) this.sharing = new Sharing(store, stateRoot);
  }
  static async load() {
  ...
  async call(name, input, onProgress, signal) {
    ...
    if (name === 'start_sharing' || name === 'create_pairing') {
      ...
      return { ...state, ...executionSettings((await this.store.read()).provider) };
    }
    ...
    if (name === 'list_models') {
      ...
      await fs.mkdir(this.sharing.root, { recursive: true, mode: 0o700 });
      return modelCapabilities(await queryModels(await codexExecutable(this.config), this.sharing.root, signal), this.config.provider);
    }
    if (name === 'caller_settings') {
      if (Object.keys(input).length) await this.store.update(config => { config.caller = { ...config.caller, ...executionSettings({ ...config.caller, ...input }), ...transferLimits({ ...config.caller, ...input }) }; });
      const saved = (await this.store.read()).caller;
      return { role: 'caller', execution: executionSettings(saved), advanced: transferLimits(saved), supported: { bytes: SUPPORT_BYTES, files: SUPPORT_FILES }, note: 'Defaults apply to new tasks. Existing tasks keep their own choices. Return limits count this transfer; input limits count the full uploaded copy.' };
    }
    if (name === 'provider_settings') {
      ...
      if (Object.keys(input).length) await this.store.update(config => {
        config.provider ||= {};
        config.provider.allowedModels = allowedModels({ ...config.provider, ...(input.allModels || input.allowedModels ? { allowedModels: input.allModels ? 'all' : input.allowedModels } : {}) });
        Object.assign(config.provider, transferLimits({ ...config.provider, ...input }));
        config.provider.retentionDays = retentionDays({ ...config.provider, ...input });
      });
      const saved = (await this.store.read()).provider;
      return { role: 'provider', allowedModels: allowedModels(saved), advanced: { ...transferLimits(saved), retentionDays: retentionDays(saved) }, supported: { bytes: SUPPORT_BYTES, files: SUPPORT_FILES, retentionDays: [1, 365] }, note: 'All dynamically includes currently available models. Model restrictions apply at every execution. Existing tasks retain accepted transfer and retention limits.' };
    }
    ...
    if (name === 'sharing_status') return { ...await this.sharing.machineStatus(), allowedModels: allowedModels(this.config.provider) };
    if (name === 'configure_model') {
      if (Object.keys(input).length) await this.store.update(config => {
        const selected = executionSettings({ ...config.provider, ...input });
    ...
    if (name === 'cleanup_shared_tasks') return this.sharing.cleanup(input.pairId, input.cleanup, signal);
    if (name === 'cleanup_shared_task') return this.sharing.cleanupTask(input, signal);
    ...
    if (name === 'list_peers') {
      const peers = await Promise.all(Object.entries(this.config.peers || {}).map(async ([name, peer]) => {
        ...
        try { const current = await connect(peer, { action: 'check' }, onProgress, signal); return { ...summary, status: current.status || 'available', limits: current.limits, checkedAt: new Date().toISOString() }; }
    ...
    if (name === 'start_task') {
      ...
      const settings = executionSettings({ ...executionSettings(this.config.caller), ...input });
      ...
      const saved = { peer: input.peer, peerId: peer.pairId || peer.id, snapshotId: input.snapshotId, ...settings, limits, createdAt: new Date().toISOString(), deliveryPending: true };
      let release;
      ...
      try {
        ...
        const result = await connect(peer, { action: 'run', protocol: 2, taskId: id, snapshot: { files: copy.files }, prompt: input.prompt, ...settings, limits }, onProgress, signal);
  ...
  async taskCall(name, input, onProgress, signal) {
    ...
    if (name === 'finish_task') {
      ...
      const finished = await connect(peer, { action: 'finish', taskId: id, revision: task.resultRevision, discardPaths: input.discardPaths, cleanup: input.cleanup }, onProgress, signal);
    ...
    if (name === 'continue_task') {
      ...
      settings = executionSettings({ ...previous, ...input });
      const capabilities = await this.capabilities(peer, onProgress, signal);
      requireModel(settings, capabilities);
    ...
    const result = await connect(peer, { action: actions[name], protocol: 2, taskId: id, prompt: input.prompt, details: input.details, ...settings, ...restoration }, onProgress, signal);
    ...
    return { taskId: id, status: result.status, model: result.model, reasoningEffort: result.reasoningEffort, error: result.error, resultDirectory: directory, responseFile: path.join(directory, 'response.md'), workCopyDirectory: path.join(workCopyRoot, 'files'), savedAt: task.savedAt, savedRevision: task.savedRevision, deliveryPending: false, changed: result.changes.files.map(f => f.path), removed: result.changes.removed, skipped: result.changes.skipped, note: 'Files and text response are saved locally. The complete task copy is reused when files have not changed. Resolve skipped necessary outputs before cleanup.' };
  }
