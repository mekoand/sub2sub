...
Use Node.js 22+ and run `npm ci --omit=optional --ignore-scripts` before testing. The runtime includes certificate generation and the Claude Agent SDK; the host uses its installed native Claude CLI, so SDK-bundled native binaries are not needed.
