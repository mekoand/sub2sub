...
`, { mode: 0o755 });
  globalThis.fetch = async url => { assert.equal(url, 'https://api.github.com/repos/mekoand/sub2sub/releases/latest'); return new Response(JSON.stringify({ tag_name: 'v0.9.0', draft: false, prerelease: false, html_url: 'https://github.com/mekoand/sub2sub/releases/tag/v0.9.0' })); };
...
test('check reports session, registered installation, node and stable release without writing', async t => {
  ...
  assert.equal(result.latest.version, '0.9.0');
...
if (!args.some(a => a.includes('/download/v0.9.0/'))) throw Error('Unexpected release');
...
data.installed[0].version = '0.9.0+codex.456';
...
`, { mode: 0o755 });
  ...
  assert.equal(result.installed.version, '0.9.0+codex.456');
  assert.equal(result.sessionVersion, JSON.parse(await fs.readFile(new URL('../package.json', import.meta.url))).version);
...
test('uncertain sharing, unknown host/root and source/newer versions never install', async t => {
  ...
  for (const [version, expected] of [['0.9.0+codex.999', 'current'], ['0.10.0+codex.999', 'newer_installed'], ['0.10.0-rc.1', 'deferred']]) {
...
test('release/network failures never become a successful check or installation', async t => {
  ...
  globalThis.fetch = async () => new Response(JSON.stringify({ tag_name: 'v0.9.0', draft: false, prerelease: true }));
  await assert.rejects(client.call('update_plugin', { action: 'install' }), /published stable release/);
  if (process.platform !== 'darwin') return;
  globalThis.fetch = async () => new Response(JSON.stringify({ tag_name: 'v0.9.0', draft: false, prerelease: false }));
  const bin = path.join(work, 'bin'); await fs.mkdir(bin);
