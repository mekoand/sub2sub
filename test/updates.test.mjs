...
const releaseVersion = JSON.parse(await fs.readFile(new URL('../package.json', import.meta.url))).version;
const newerVersion = `${Number(releaseVersion.split('.')[0]) + 1}.0.0`;
...
`, { mode: 0o755 });
  globalThis.fetch = async url => { assert.equal(url, 'https://api.github.com/repos/mekoand/sub2sub/releases/latest'); return new Response(JSON.stringify({ tag_name: `v${releaseVersion}`, draft: false, prerelease: false, html_url: `https://github.com/mekoand/sub2sub/releases/tag/v${releaseVersion}` })); };
...
test('check reports session, registered installation, node and stable release without writing', async t => {
  ...
  assert.equal(result.latest.version, releaseVersion);
...
if (!args.some(a => a.includes('/download/v${releaseVersion}/'))) throw Error('Unexpected release');
...
data.installed[0].version = '${releaseVersion}+codex.456';
...
`, { mode: 0o755 });
  ...
  assert.equal(result.installed.version, `${releaseVersion}+codex.456`);
  assert.equal(result.sessionVersion, JSON.parse(await fs.readFile(new URL('../package.json', import.meta.url))).version);
...
test('uncertain sharing, unknown host/root and source/newer versions never install', async t => {
  ...
  for (const [version, expected] of [[`${releaseVersion}+codex.999`, 'current'], [`${newerVersion}+codex.999`, 'newer_installed'], [`${newerVersion}-rc.1`, 'deferred']]) {
...
test('release/network failures never become a successful check or installation', async t => {
  ...
  globalThis.fetch = async () => new Response(JSON.stringify({ tag_name: `v${releaseVersion}`, draft: false, prerelease: true }));
  await assert.rejects(client.call('update_plugin', { action: 'install' }), /published stable release/);
  if (process.platform !== 'darwin') return;
  globalThis.fetch = async () => new Response(JSON.stringify({ tag_name: `v${releaseVersion}`, draft: false, prerelease: false }));
  const bin = path.join(work, 'bin'); await fs.mkdir(bin);
