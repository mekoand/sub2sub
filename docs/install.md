...
有活动任务、尚未保存或状态不确定的本地委托时，升级暂缓。先等工作结束，查询任务状态并保存成果后重试；升级不会取消任务或清理用户数据。安装保留配对、证书、配置、任务数据和本地成果，无需搬迁这些数据。失败信息标明查询或下载/安装环节；若安装曾中断，先核查宿主插件列表，不能把安装尝试当作成功。

安装完成且宿主登记验证通过后，Codex 桌面用户完整退出并重新打开应用；仅关闭窗口或新建对话不足以确保加载新版。CLI 用户退出并重启 `codex` 或 `claude`。安装登记成功后，能够确认空闲的独立节点会从已安装的新版入口重新启动，保留原接单或暂停状态；原来没有运行的节点不会自动启动。有活动任务、在途传输或无法确认状态时保留节点并说明原因。旧节点若不支持完整的在途传输检查，则先完成工作、明确退出节点，再重启管理应用并开启共享。**从 0.5.3 升级时，共享进程依附旧共享端对话，没有新版的独立节点退出命令；先完成任务、保存成果，再结束旧共享端对话，由新对话开启共享。** 0.5.3 本身没有对话升级工具，首次升到提供该工具的版本仍需运行本文安装命令。身份未改变时无需重新配对；两端沿用现有能力检查，不保证任意新旧版本互通。

安装完成和插件启动时，会检查旧程序目录。当前程序、其他应用仍登记使用的版本、进程占用的版本和配置的数据路径会保留；无法确认引用或删除失败时也保留并说明。应用退出或重启后可以完成余下清理；当前安装已是最新版时，再次要求升级会重试节点切换与清理，不重复下载。升级结果分别说明安装、节点切换以及已删除、保留和失败的程序目录；启动清理结果也可通过 `sharing_status` 查看。当前对话不会因此变成新版。
...
程序、任务记录和本地成果是分开的。卸载插件不会替你删除成果；任务与原生会话的删除按[清理选择](usage.md#任务生命周期)处理。需要释放旧程序占用时，先退出旧会话，再启动新版插件或明确重试升级，由现有引用检查完成清理。不要手动删除仍被其他应用或进程使用的版本。
