...
共享端在能力查询中声明 gzip 支持后，使用端对文件上传使用低级别流式 gzip；成果响应按请求的 `Accept-Encoding` 选择压缩。压缩只包裹现有记录流，沿用背压和解压后的原始文件限制，完整压缩流校验通过后才能执行或保存。旧端未声明支持时保持未压缩上传，不因压缩失败重放任务。本机转发的本地一跳不压缩，远端一跳按同样的协商规则处理；stdio 不压缩。小文件准备和初次落盘合并重复目录创建，不加入并发扫描或长期缓存。合成对照与限制见[传输性能测量](development/transfer-performance.md)。
...
同一任务的续作可显式携带冻结快照与逐文件删除列表。使用端对照已确认完整副本计算文件差异；共享端以既有修订号、保存确认及文件清单摘要核对基准，并检查工作文件是否在确认后发生变化。在任务锁内准备临时完整副本，成功后替换工作目录再执行；结果仍对比原确认基准，因此取回包含本地上传和共享端后续修改。没有长期项目同步、合并引擎或新缓存。扫描、哈希和暂存仍有本地成本。

替换期间沿用任务状态记录 `updating`，保留原工作副本供失败恢复。任务锁记录进程归属，只有确认旧进程退出后才回收；旧的无归属锁仍需共享端人工检查。下次显式操作可恢复中断的替换，但不自动执行，先取回结果。已清理任务带文件续作时，内部先完整恢复而不执行，再单独发送增量并执行，避免同一请求重复计算完整副本与变更文件的传输上限。
