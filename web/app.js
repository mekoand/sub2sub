...
async function settings() {
  ...
  mount.append(el('section',{class:'setting'},el('h2',{},t('跨网络连接','Cross-network connections')),el('p',{class:'sub'},guide.settings.crossNetwork.enabled?t('当前已开启','Currently on'):t('当前已关闭','Currently off')),el('p',{},t('默认关闭。开启后可连接其他网络中的已授权设备，双方都需要开启。系统优先尝试直接连接，必要时使用公共中继；中继可能影响速度与可用性。文件传输仍需单独授权。','Off by default. Enable on both devices to connect across networks. Direct connections are tried first; public relays may affect speed and availability. Task-file permission remains separate.')),form([check(t('开启跨网络连接服务','Enable cross-network service'),'crossNetwork',guide.settings.crossNetwork.enabled)],async values=>{try{await call('device_settings',{crossNetwork:values.has('crossNetwork')});}catch(error){const actual=await call('device_settings',{});mount.querySelector('[name=crossNetwork]').checked=actual.crossNetwork.enabled;throw error;}await settings();say(t('跨网络连接设置已保存','Cross-network setting saved'));})));
...
function showOnboarding(data) {
  ...
  const rows=[
    ...
    [t('跨网络连接','Cross-network connections'),s.crossNetwork.enabled?t('已开启','On'):t('已关闭','Off')],
