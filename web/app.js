...
function taskDetail(selected) {
  ...
  return el('section',{class:'detail'},el('div',{class:'sectionhead'},el('h2',{},t('任务详情','Task details')),btn('×',()=>{state.selected=null;render();},'close')),el('dl',{},rows.flatMap(([a,b])=>[el('dt',{},a),el('dd',{},b)])),current.error?el('p',{class:'error'},current.error):null,saved?.delivery?el('div',{class:'info'},el('p',{},`${t('本次保存','Saved delivery')} · ${labelStatus(saved.delivery.status)} · ${t('第','Revision')} ${saved.delivery.revision} ${t('轮','')}`),saved.delivery.stopReason==='time_limit'?el('p',{},t('达到执行时限，仅保存阶段成果。','Execution reached its deadline; stage results only.')):null,saved.delivery.error?el('p',{class:'error'},saved.delivery.error):null,saved.delivery.skipped.length?el('div',{},el('p',{},t('以下路径未包含在成果中。必要文件需补齐后再清理。','These paths are missing from the delivery. Recover necessary files before cleanup.')),el('ul',{},saved.delivery.skipped.map(name=>el('li',{},name)))):null):null,el('div',{class:'actions'},controls),saved?.workCopyDirectory?el('p',{class:'file-path'},saved.workCopyDirectory):null,
...
const consentText=()=>t('以后委托给此设备的任务会发送所需文件与指令，包括必要的非公开源码、文档和配置；凭据、无关文件及需单独授权的敏感材料不在此范围。此设置不替代宿主审批。','Delegated tasks send necessary files and instructions, including private source, documents and configuration. Credentials, unrelated files and separately sensitive material are excluded. Host approval still applies.');
...
function resources() {
  ...
  const shown=state.role==='caller'?peers:[{name:state.data.device,status:sharing.status,harness:sharing.harness}];
...
async function settings() {
  ...
  mount.replaceChildren(el('section',{class:'setting'},el('div',{class:'sectionhead'},el('h2',{},t('网页管理','Web management')),btn(t('关闭','Disable'),()=>confirmation(t('关闭网页管理','Disable web management'),t('只停止本机管理入口，独立共享继续。之后在原对话中说“打开管理页”即可重新启用。','Stops this local management entry; independent sharing continues. Ask to open management in the original conversation to enable it again.'),async()=>{await call('web_management',{enabled:false});state.stopped=true;clearTimeout(poll);app.replaceChildren(el('main',{},el('h1',{},t('网页管理已关闭','Web management disabled')),el('p',{class:'info'},t('可在原对话中重新打开。独立共享不受影响。','Reopen it from the original conversation. Independent sharing is unaffected.'))));}))),el('p',{},t('默认开启，仅本机访问。完整退出宿主后停止；不会自动打开浏览器。','Enabled by default on this computer. Ends with its host process; never opens a browser automatically.'))),
    ...
    el('section',{class:'setting'},el('h2',{},t('本机提供的资源','Resources I provide')),form([field(t('执行工具','Execution tool'),'harness',provider.harness,'select',[['codex','Codex'],['claude','Claude Code']]),check(t('开放全部可用模型，包含以后新增的模型','Offer all available models, including future additions'),'all',provider.allowedModels==='all'),field(t('指定模型，每行一个；开放全部时忽略','Specific models, one per line; ignored when all are offered'),'models',Array.isArray(provider.allowedModels)?provider.allowedModels.join('\n'):'','textarea'),advancedFields(provider.advanced,true)],async values=>{
      ...
      await call('provider_settings',input);guide.settings.provider.allowedModels[input.harness]=input.allModels?'all':input.allowedModels;state.quotas.delete('$local');say(t('提供方设置已保存','Provider settings saved'));
...
function render() {
  ...
  else if(state.data)main.append(resources(),el('section',{},el('div',{class:'sectionhead'},el('h2',{},t('最近任务','Recent tasks')),el('span',{class:'sub'},t('按最近更新时间','Recently updated'))),state.data.tasks.length?state.data.tasks.map(taskSummary):el('p',{class:'empty'},t('还没有任务记录。','No task records yet.'))));
...
async function refresh(check=false) {
  ...
  try {
    ...
    if(role==='caller')data.peers=data.peers.map(peer=>{
      ...
      return saved?{...peer,status:saved.status,harness:saved.harness,reason:saved.reason,checkedAt:saved.checkedAt}:peer;
    });
