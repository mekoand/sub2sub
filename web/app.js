...
async function storage() {
  ...
  showDialog(t('本地空间','Local storage'),el('div',{},el('p',{},`${t('调用方文件总计','Caller file total')} ${bytes(data.bytes)}`),el('p',{class:'sub'},t('同一目录在总计中只计一次。大小按文件字节统计，实际释放空间由文件系统决定。','Shared directories count once in the total. File sizes may differ from disk space reclaimed.')),data.errors.map(error=>el('p',{class:'error'},`${error.path}: ${error.reason}`)),rows.length?rows:el('p',{},t('暂无任务文件','No task files')),data.snapshots.map(entry=>el('div',{class:'setting'},el('p',{class:'info'},`${t('未使用的输入快照','Unused input snapshot')} · ${bytes(entry.bytes)}\n${entry.path}`),btn(t('清理快照','Clear snapshot'),()=>previewStorage({snapshotId:entry.snapshotId,scope:'all'}),'link'))),el('p',{class:'sub'},t('提供方工作副本使用原有任务清理入口，保留期不变。','Provider work copies use existing task cleanup controls; retention is unchanged.')),btn(t('查看提供方工作副本占用','Inspect provider work copies'),async()=>{
    const result=await call('list_shared_tasks',{details:true});
    showDialog(t('提供方工作副本','Provider work copies'),el('div',{},result.tasks.length?result.tasks.map(task=>el('p',{class:'info'},`${task.taskId} · ${bytes(task.inspection?.bytes)}${task.inspection?.error?'\n'+task.inspection.error:''}`)):el('p',{},t('暂无工作副本','No work copies')),btn(t('返回本地空间','Back to local storage'),storage)));
  },'link')));
}
...
async function settings() {
  ...
  mount.prepend(el('section',{class:'setting'},el('h2',{},t('设备名称','Device name')),el('p',{},t('仅用于 sub2sub。对端在后续通信时更新，手动连接别名保留。','Used by sub2sub only. Paired devices update on subsequent contact; explicit aliases remain.')),form([field(t('本机显示名','Device display name'),'name',guide.settings.deviceName)],async values=>{const saved=await call('device_settings',{name:values.get('name')});if(state.data)state.data.device=saved.deviceName;await settings();say(t('设备名称已保存','Device name saved'));})));
  mount.append(el('section',{class:'setting'},el('h2',{},t('跨网络连接','Cross-network connections')),el('p',{class:'sub'},guide.settings.crossNetwork.enabled?t('当前已开启','Currently on'):t('当前已关闭','Currently off')),el('p',{},t('默认关闭。开启后可连接其他网络中的已授权设备，双方都需要开启。系统优先尝试私网直连；跨网服务依赖公共发现与中继，可能影响速度与可用性。关闭前须完成或明确取消跨网任务及传输。文件授权和任务联网权限保持原设置。','Off by default. Enable on both devices to connect across networks. Private connections are tried first. Cross-network service relies on public discovery and relays, which may affect speed and availability. Finish or explicitly cancel cross-network work and transfers before disabling. File consent and task network permissions remain separate.')),form([check(t('开启跨网络连接服务','Enable cross-network service'),'crossNetwork',guide.settings.crossNetwork.enabled)],async values=>{try{await call('device_settings',{crossNetwork:values.has('crossNetwork')});}catch(error){const actual=await call('device_settings',{});mount.querySelector('[name=crossNetwork]').checked=actual.crossNetwork.enabled;throw error;}await settings();say(t('跨网络连接设置已保存','Cross-network setting saved'));})));
  mount.append(el('section',{class:'setting'},el('h2',{},t('本地空间','Local storage')),el('p',{},t('手动查看占用，清理旧成果或全部本地文件。不会自动清理。','Inspect sizes and remove old or all local files manually. No automatic cleanup.')),btn(t('查看空间占用','Inspect storage'),storage)));
