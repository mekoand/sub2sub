...
- `sharing_status`: this machine's actual sharing process and active task. It reports the node and current management session versions; `update_plugin(action=check)` reads the host's actual installed version. `stop_sharing` pauses new execution; existing tasks may finish and results remain accessible. Closing any management conversation leaves an independent node running. `exit_sharing` fully stops an idle node; finish or explicitly cancel active work first. After an update, use these actions from a new session when idle; do not remove a program version still used by the node. For 0.5.3's conversation-owned sharing, follow the installation guide instead of the newer exit action.
