...
- **View saved results:** read the [local delivery format](references/work-copy.md#collect-and-continue), call `list_tasks`, identify the task by peer and times, then read/open its local `workCopyDirectory`, `responseFile` and `resultDirectory/changes.json`. Present local file links and the saved text answer together; disclose failed/interrupted execution and any skipped necessary outputs from the saved manifest. This works after remote cleanup or disconnection. Viewing does not call `collect_result`, `task_status`, SSH or another remote access tool. If `deliveryPending=true`, explain that the listed files are the previous save; absent freshness information in older records is unknown.
