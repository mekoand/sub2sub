...
async function installClaude(payload, root, release, run, log, executable) {
  ...
  try {
    ...
    await fs.writeFile(path.join(adapter, '.claude-plugin/plugin.json'), JSON.stringify({ name: 'sub2sub', version, description: "A safer, lighter, smoother way to share AI resources and work together.", license: 'MIT' }, null, 2) + '\n');
