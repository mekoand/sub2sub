...
async function installClaude(payload, root, release, run, log, executable) {
  ...
  try {
    ...
    await fs.writeFile(path.join(adapter, '.claude-plugin/plugin.json'), JSON.stringify({ name: 'sub2sub', version, description: "Delegate AI tasks across your devices or team, receive files and responses, and continue in the same conversation.", license: 'MIT' }, null, 2) + '\n');
