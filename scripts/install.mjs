...
async function installClaude(payload, root, release, run, log, executable) {
  ...
  try {
    ...
    for (const name of ['skills', 'docs', 'README.md', 'README.zh-CN.md', 'README.en.md', 'CHANGELOG.md', 'CONTRIBUTING.md', 'CONTRIBUTING.zh-CN.md', 'SECURITY.md', 'LICENSE']) await fs.cp(path.join(plugin, name), path.join(adapter, name), { recursive: true });
    const version = `${release.version}+claude.${Date.now()}`;
    await fs.writeFile(path.join(adapter, '.claude-plugin/plugin.json'), JSON.stringify({ name: 'sub2sub', version, description: "Share Codex and Claude Code subscriptions across devices through a task proxy.", license: 'MIT' }, null, 2) + '\n');
