...
const prompt = { ...string('Instructions for the remote Codex session.'), maxLength: 100000 };
...
const definitions = [
  ['caller_settings', '使用方普通设置：查询或修改所有节点共用的默认模型和思考强度。只影响新任务，单次任务可覆盖。', { model: string('Default Codex model ID.'), reasoningEffort: { ...string('Default reasoning effort.'), enum: REASONING_EFFORTS }, ...advancedLimits }, []],
  ['provider_settings', '提供方普通设置：查询或修改开放模型，默认全部并包含新增模型。高级设置包括传输限制和任务保留天数；默认闲置 7 天，旧任务保留原期限。', { allModels: { type: 'boolean', description: 'True to provide all currently available models.' }, allowedModels: { type: 'array', minItems: 1, maxItems: 2000, items: string('Allowed model ID.') }, ...advancedLimits, retentionDays: { type: 'integer', minimum: 1, maximum: 365, description: '高级设置：从最后一轮结束起保留的天数，默认 7 天。只有已确认保存且无执行中的任务才自动清理。' } }, []],
  ...
  ['start_sharing', 'Enable this device to execute authorized LAN peer tasks using its own Codex subscription. The listener lives in this plugin process; no login startup item is installed. Select a LAN address from setup_status if needed.', { address: string('Local private IPv4 address to listen on.'), port: { type: 'integer', minimum: 0, maximum: 65535, description: 'Listening port. Default 47631; zero chooses an available port.' } }, []],
  ['sharing_status', '提供方：查询本机实际共享进程及活动任务，包含检查时间；不同 Codex 对话看到同一共享状态。', {}, []],
  ...
  ['create_pairing', 'Generate an invitation to share this device’s Codex. Starts sharing if needed; call directly without setup/status checks. One use, valid for 10 minutes. Keep this Codex task open and send the invitation privately.', { address: string('Optional private IPv4 address. Auto-selected on a single LAN interface.'), port: { type: 'integer', minimum: 0, maximum: 65535, description: 'Optional port; defaults to 47631. Zero chooses an available port.' } }, []],
  ...
  ['list_tasks', '查看本地委托任务，含创建/结束时间、保存时间、完整文件目录和文字答复 responseFile。按时间排列；deliveryPending 表示本轮尚未保存。查看成果只读取这些本地路径，无需连接远端或 collect_result；远端状态另用 task_status。', {}, []],
  ...
  ['edit_peer', '使用方：重命名连接或更新同一设备的 LAN 地址，保留任务关联和传输授权。地址更新先验证原配对身份；更换设备身份需重新配对。', { peer: string('Existing connection name.'), name: string('Optional new connection name.'), host: string('Optional new private IPv4 address for the same device.'), port: { type: 'integer', minimum: 1, maximum: 65535 } }, ['peer']],
...
lines.on('line', async line => {
  ...
  try {
    ...
    if (message.method === 'initialize') {
      initialized = true;
      send({ id: message.id, result: { protocolVersion: '2024-11-05', capabilities: { tools: {} }, serverInfo: { name: 'sub2sub', version: '0.4.3' } } });
      return;
    }
